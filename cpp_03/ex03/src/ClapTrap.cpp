...
void	ClapTrap::takeDamage(unsigned int amount)
{
	if (this->pv > 0)
	{
		if (this->pv < amount)
			...
			this->pv -= amount;
		std::cout << "\033[1;33m" << this->name << " has " << this->pv << " health points left." << "\033[0m" << std::endl;
	}
	else if  (this->pv == 0)
		std::cout << "\033[1;33m" << this->name << " is already dead, please stop harping." << "\033[0m" << std::endl;
}
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	if (this->e > 0 && this->pv > 0)
	{
		this->e -= 1;
		this->pv += amount;
		std::cout << "\033[1;33m" << this->name << " recovered " << amount << " health points." << "\033[0m" << std::endl;
	}
	else if (this->pv == 0)
		std::cout << "\033[1;33m" << "Sorry, " << this->name << " is dead, and we're not creating a zombie." << "\033[0m" << std::endl;
	else if (this->e == 0)
		std::cout << "\033[1;33m" << "Sorry, " << this->name << " has no health points left." << "\033[0m" << std::endl;
}
